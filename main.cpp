...
public:
  ...
  SDL_Texture *textureUp, *textureDown, *textureLeft, *textureRight,
      *currentTexture; // Textures for 4 directions
  ...
  Bullet(int startX, int startY, int dirX, int dirY, SDL_Renderer *ren)
    ...
    textureUp = IMG_LoadTexture(
        renderer, "bullet_up.png"); // Replace with your image file names
...
public:
  ...
  int dirX, dirY;
  Mix_Chunk *shootSound; // Sound effect for shooting

  PlayTank(int startX, int startY, SDL_Renderer *ren, vector<Wall> *wallList,
           vector<Bullet> *bulletList)
      : x(startX), y(startY), renderer(ren), walls(wallList),
        bullets(bulletList), dirX(0), dirY(-1) {
    rect = {x, y, TILE_SIZE, TILE_SIZE};
    ...
    if (!shootSound) {
      cout << "Failed to load shoot sound: " << Mix_GetError() << endl;
    }
  }
  ...
  void handleEvent(SDL_Event &e) {
    if (e.type == SDL_KEYDOWN) {
      ...
      case SDLK_RIGHT:
        ...
        break;
      case SDLK_SPACE:
        bullets->emplace_back(x + TILE_SIZE / 2 - 8, y + TILE_SIZE / 2 - 8,
                              dirX, dirY, renderer);
        if (shootSound) {
      ...
      if (!checkCollision(newX, newY, *walls) && newX >= 0 && newY >= 0 &&
          newX + TILE_SIZE <= SCREEN_WIDTH &&
          newY + TILE_SIZE <= SCREEN_HEIGHT) {
        x = newX;
        y = newY;
        rect.x = x;
        rect.y = y;
...
public:
  ...
  bool checkCollision(int newX, int newY, const vector<Wall> &walls,
                      const vector<EnemyTank> &enemies) {
    SDL_Rect newRect = {newX, newY, TILE_SIZE, TILE_SIZE};

    for (const auto &wall : walls) {
      if (SDL_HasIntersection(
              &newRect,
              &wall.rect)) { // Kiểm tra va chạm với tất cả các loại tường
        return true;
      }
    }
...
public:
  ...
  Game() {
    ...
    if (!boomSound) {
        cout << "Failed to load boom sound: " << Mix_GetError() << endl;
    }
  ...
  void generateWalls() {
    ...
    string mapLayout[7] = {
        ...
        "CCBBBW   WCCBBC ", "B    B   BB    ", "B    B   C    ",
        "WBCCB    W    ",   "B    B   C    ",  "B   WB   CBW   ",
        "CCBBBW   BBCCBB "};
                  ...
                  [&](Bullet &b) {
                      for (auto it = walls.begin(); it != walls.end(); ++it) {
                          if (it->type == 0 &&
                              ...
                              walls.erase(it);
                              if (boomSound) { // Kiểm tra boomSound
                                  Mix_PlayChannel(-1, boomSound, 0); // Phát âm thanh
                              }
                              return true;
                          } else if (it->type == 1 &&
                                     SDL_HasIntersection(&b.rect, &it->rect)) {
  ...
  void renderGameOver() {
    if (gameOverTexture) {
      SDL_Rect gameOverRect = {SCREEN_WIDTH / 2 - 200, SCREEN_HEIGHT / 2 - 100,
                               400, 200}; // Điều chỉnh vị trí và kích thước
      SDL_RenderCopy(renderer, gameOverTexture, nullptr, &gameOverRect);
    }
  }
  void renderYouWin() {
    if (youWinTexture) {
      SDL_Rect youWinRect = {SCREEN_WIDTH / 2 - 200, SCREEN_HEIGHT / 2 - 100,
                             400, 200}; // Điều chỉnh vị trí và kích thước
